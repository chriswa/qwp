...
export function createInterpreter(path: string, source: string, isDebug: boolean): IInterpreterFacade {
...
export class Interpreter implements IInterpreterFacade {
  ...
  pushScope(node: SyntaxNode): void {
  ...
  runUntilBudgetExhausted(budget: number): number {
    ...
    while (remainingBudget > 0) {
      remainingBudget -= this.runOneStep()
    }
