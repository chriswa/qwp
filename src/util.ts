...
type MapVariadicArraysToElements<T extends unknown[][]> = { [I in keyof T]: T[I] extends (infer U)[] ? U : never }
export function zipMap<T extends unknown[][], U>(arrays: [...T], transformer: (...args: MapVariadicArraysToElements<T>) => U): Array<U> {
  if (arrays.length === 0) { return [] }
  const lengths = arrays.map((array) => array.length)
  if (lengths.every((length) => length === lengths[0]) === false) {
    throw new InternalError(`zipMap called with arrays which have different lengths`)
  }
  const iterator: MapVariadicArraysToElements<T> = [] as unknown[] as MapVariadicArraysToElements<T>
